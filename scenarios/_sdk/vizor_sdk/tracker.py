"""Generic multi-object ByteTrack + Kalman tracker for scenario plugins.

Tracks any bounding boxes (faces, persons, vehicles, objects). Pure numpy, no
behavioural change from the proven FRS port.
...
A worker tracks detections in detection order, so `assign_track_ids` adapts the
...
def assign_track_ids(tracker: "ByteTracker", dets: list) -> list[int]:
    """Adapter for the worker: `dets` is a per-object list of (bbox_xyxy, conf).
