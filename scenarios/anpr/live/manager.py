...
to the NVR so the Cameras tab shows running / stopped / error. Mirrors PPE/FRS.
"""
...
def _report_state(config_id, state, error):
    ...
    try:
        ...
        print(f"[anpr-live] state report failed ({state}) for {config_id}: {exc}", flush=True)
    ...
    with _LOCK:
        ...
        for cam_id in list(_WORKERS):
            if cam_id not in cams:
                ...
                w.stop()
                print(f"[anpr-live] stopped worker for {cam_id}", flush=True)
        ...
        for cam_id, cam in cams.items():
            ...
            if existing and existing.is_alive():
                if _cfg_sig(existing.config) != _cfg_sig(cam.get("config")):
                    ...
                    _WORKERS[cam_id] = nw
                    nw.start()
                    print(f"[anpr-live] restarted worker (config change) for {cam_id}", flush=True)
                continue
            w = CameraWorker(cam, _report_state)
            _WORKERS[cam_id] = w
            w.start()
            print(f"[anpr-live] started worker for {cam_id}", flush=True)
...
def _loop():
    while True:
        try:
            ...
            print(f"[anpr-live] reconcile failed: {exc}", flush=True)
        time.sleep(config.LIVE_POLL_SECONDS)


def live_status() -> dict:
    """Snapshot of worker liveness for /health."""
    now = time.time()
    with _LOCK:
...
def start_live_manager():
    ...
    if not config.LIVE_ENABLED:
        print("[anpr-live] live ANPR disabled (ANPR_LIVE_ENABLED=false)", flush=True)
        return
    threading.Thread(target=_loop, daemon=True, name="anpr-live-manager").start()
    print("[anpr-live] live manager started", flush=True)
