...
Periodically purges ANPR plate reads older than RETENTION_EVENT_DAYS along with
their snapshot files. 0 days disables purging. Mirrors PPE.
"""
...
def _purge_old_events() -> int:
    ...
    while True:
        with session() as s:
            rows = s.execute(
                select(ANPRPlateRead).where(ANPRPlateRead.triggered_at < cutoff)
...
def _loop() -> None:
    ...
    while True:
        try:
            ...
            if purged:
                print(f"[anpr-retention] purged {purged} old reads", flush=True)
        except Exception as exc:  # noqa: BLE001
            print(f"[anpr-retention] sweep error: {exc}", flush=True)
        time.sleep(interval)
...
def start_retention_sweeper() -> None:
    ...
    if _started:
        return
    _started = True
    threading.Thread(target=_loop, daemon=True, name="anpr-retention").start()
    print("[anpr-retention] sweeper started", flush=True)
