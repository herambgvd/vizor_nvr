...
async def get_snapshot(
    ...
    """Proxy a PPE event snapshot through the bridge to the scenario's rustfs.
    ...
    back. The bridge emits snapshot_path="/api/ai/ppe/snapshot?key=..." so this
    ...
    try:
        async with httpx.AsyncClient(timeout=_IMAGE_TIMEOUT) as client:
            resp = await client.get(
                f"{BRIDGE_HTTP_URL}/snapshot/ppe", params={"key": key}
            )
    except httpx.HTTPError as e:
        raise _bridge_error("failed to reach bridge", e)
    if resp.status_code >= 400:
