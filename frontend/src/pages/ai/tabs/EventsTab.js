...
function SnapshotThumb({ ev, slug }) {
  ...
  // Prefer the cropped detected face for the table thumbnail (full frame stays
  // in the detail modal); fall back to the full-frame snapshot.
...
function EventDetailModal({ event, slug, onClose }) {
  ...
  const ev = event;
  const faceSnap = ev.attributes?.face_snapshot;
  const matchedPhotoId = ev.attributes?.matched_photo_id;
      ...
      <div className="w-full max-w-3xl rounded-lg flex flex-col" style={{ background: "var(--console-panel)", border: "1px solid var(--console-border)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4">
          ...
          <div className="md:col-span-3">
            <div className="font-telemetry text-[9px] uppercase tracking-widest mb-1.5" style={{ color: "var(--console-muted)" }}>Full frame</div>
            <AuthImage
...
export default function EventsTab({ scenario }) {
  ...
  // ── Selection + delete (FRS plugin owns its events) ──────────────────────
  ...
  // Delete routes to the right store: FRS owns its events in the plugin DB;
  // other scenarios live in the unified NVR event store.
  const delMutation = useMutation({
    mutationFn: (eventId) => (isFrs ? deleteFrsEvent(eventId) : deleteEvent(eventId)),
  ...
  const bulkDelMutation = useMutation({
    mutationFn: (ids) =>
      isFrs ? bulkDeleteFrsEvents({ ids }) : bulkDeleteEvents({ event_ids: ids }),
    onSuccess: (r) => {
  ...
  return (
    <div className="p-4 space-y-3">
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={9} className="px-3 py-12 text-center text-sm text-rose-400">
              ...
              items.map((ev) => (
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    <SnapshotThumb ev={ev} slug={scenario?.slug} />
                  </td>
                  <td className="px-3 py-2">
      ...
      {detailEvent && (
        <EventDetailModal event={detailEvent} slug={scenario?.slug} onClose={() => setDetailEvent(null)} />
      )}
    </div>
  );
}
