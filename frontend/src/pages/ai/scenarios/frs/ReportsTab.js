// =============================================================================
// AI · FRS · Reports tab — face-recognition summary, demographics and
// confidence distribution. Scoped to FRS only.
//   • Events read from the FRS plugin store (listFrsEvents).
//   • Plugin /reports/summary drives recognition totals + hourly distribution.
// =============================================================================
...
import { listScenarioCameras, scenarioReportsSummary } from "../../../../api/ai";
...
export default function ReportsTab({ scenario }) {
  ...
  const { data: events, isLoading: eventsLoading, isFetching } = useQuery({
    queryKey: ["scenario-events-summary", "frs", since, until],
    queryFn: () => listFrsEvents({ since, until, limit: 500 }),
    enabled: !!scenario?.slug,
    placeholderData: keepPreviousData,
  });

  const { data: pluginSummary } = useQuery({
    queryKey: ["scenario-plugin-summary", "frs", since, until],
  ...
  const ageRangeData = useMemo(() => {
    ...
    eventsList.forEach((event) => {
      ...
      if (!ageRange) return;
      counts.set(String(ageRange), (counts.get(String(ageRange)) || 0) + 1);
    });
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          ...
          value={eventsLoading ? "—" : (pluginSummary?.total_events ?? events?.total ?? eventsList.length)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Panel title="Events by camera" icon={BarChart3}>
          {eventsLoading ? (
            ...
            <HBarChart data={byCameraData} color="#228B22" emptyLabel="No scenario events in this range yet." />
          )}
        </Panel>
      ...
      {/* Recognition summary — counts + hourly distribution. */}
      {pluginSummary && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      {/* Demographics — gender + age-range distribution from event attributes. */}
      {hasDemographics && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {/* Confidence histogram — distribution of recognition confidence. */}
