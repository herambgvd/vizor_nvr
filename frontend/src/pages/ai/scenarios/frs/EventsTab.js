// =============================================================================
// AI · FRS · Events tab — face-recognition event log.
//
// Scoped to FRS only. Reads the rich FRS plugin event store (listFrsEvents,
// with person joins), shows the recognition columns (time / camera / type /
// person+conf / detected face / matched POI), and opens a full-detail modal
// with demographics, the matched POI photo and operator correct/wrong feedback.
// =============================================================================
...
function SnapshotThumb({ ev }) {
  ...
  // Prefer the cropped detected face for the table thumbnail; fall back to the
  // full-frame snapshot.
...
// Authenticated <img> for a plugin-relative snapshot path or person photo.
...
// Full-detail modal opened by clicking an event row. FRS keeps the rich
// face/match/demographic set plus operator correct/wrong feedback.
function EventDetailModal({ event, camMap, onClose }) {
  const [verdict, setVerdict] = useState(null);
  const submitVerdict = async (isCorrect) => {
    setVerdict(isCorrect);
    try {
      await submitFrsFeedback({ event_id: event.id, is_correct: isCorrect, matched_person_id: event.person_id });
      ...
      <div className="w-full max-w-3xl rounded-lg flex flex-col" style={{ background: "var(--console-panel)", border: "1px solid var(--console-border)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4">
          ...
          <div className="md:col-span-2 flex flex-col gap-3">
            ...
            {/* Operator feedback — was the recognition right? */}
            {ev.person_id && (
...
export default function EventsTab({ scenario }) {
  ...
  const delMutation = useMutation({
    mutationFn: (eventId) => deleteFrsEvent(eventId),
  ...
  const bulkDelMutation = useMutation({
    mutationFn: (ids) => bulkDeleteFrsEvents({ ids }),
    onSuccess: (r) => {
  ...
  const hasFilters = cameraId !== ALL || eventType !== ALL || personId.trim() || since || until;
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-lg border p-3 shrink-0" style={{ borderColor: "var(--console-border)", background: "var(--console-panel)" }}>
        ...
        <div className="w-44">
          <Input
            ...
            value={personId}
            onChange={(e) => { setPersonId(e.target.value); setPage(0); }}
          />
        </div>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="px-3 py-16 text-center">
                  ...
                  <p className="text-xs text-zinc-500 mt-1">
                    {hasFilters ? "Try widening your filters." : "Events appear here as faces are recognized."}
                  </p>
                </td>
              </tr>
            ) : (
              items.map((ev) => (
                <tr key={ev.id} onClick={() => setDetailEvent(ev)} className="border-t hover:bg-white/[0.04] transition-colors cursor-pointer" style={{ borderColor: "var(--console-border)" }}>
                  ...
                  <td className="px-3 py-2">
                    <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-[11px] font-medium", eventTypeBadgeClass(ev.event_type))}>
