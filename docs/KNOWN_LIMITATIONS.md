...
| PPE compliance accuracy | ⚠️ | Pipeline ported from a proven POC (temporal grace, smoothing, occlusion re-link, body-zone assignment) on Triton (`ppe_yolo26`). **Not yet validated on real site footage in-platform.** DINOv2 second-stage verifier is deferred (YOLO-only baseline runs; hook in place). Thresholds are POC-tuned to one camera. |
| ANPR licensing | 🔒 | ANPR registers + runs, but `anpr` is **not in the signed license** yet, so it is not auto-licensed in production (dev override used for testing). Add `anpr` to the license feature set to ship it. |
